  ...
  if (!item?.slot) {
    return (
      <section className={cn(card, 'p-4')}>
        ...
        <h3 className="text-[22px] font-bold leading-tight tracking-[-0.02em] text-[var(--text)]">Вы пока не записаны</h3>
        <p className="mt-2 text-[15px] font-medium leading-5 text-[var(--text-soft)]">Выберите свободное окно ниже или откройте расписание</p>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[19px] font-bold leading-6 tracking-[-0.02em] text-[var(--text)]">{slotTimeRange(slot)}</p>
...
export function LessonDetailsCard({ title, item }: { title: string; item: ResolvedStudentBooking | null }) {
  ...
  if (!item?.slot) {
    return (
      <section className={cn(card, 'p-4')}>
        <h2 className="text-[18px] font-bold tracking-[-0.02em] text-[var(--text)]">{title}</h2>
  ...
  return (
    <section className={cn(card, 'p-4')}>
      ...
      <div className="mt-4 rounded-[18px] bg-[var(--surface-soft)] p-3">
        <p className="text-[15px] font-bold text-[var(--text)]">{description?.theme ?? 'Тему уточнит инструктор'}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {(description?.whatToBring?.length ? description.whatToBring : ['Паспорт', 'Удобная обувь']).map((item) => <span key={item} className="rounded-full bg-[var(--accent-soft)] px-3 py-1.5 text-[12px] font-bold text-[var(--accent)]">{item}</span>)}
      </div>
    </section>
  )
}
